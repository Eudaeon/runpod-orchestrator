...
async def get_custom_docker_args(client: RunpodClient, template_id: str, s) -> str:
    ...
    original_args = await client.get_pod_template_args(template_id)
...
async def orchestrate(
    ...
    pod_id, start_time, proc, pod_info = None, None, None, None
    exit_code = 0
        ...
        try:
            with logger.status(f"Deploying {name}...") as s:
                try:
                    ...
                    pod_id = await client.deploy_pod(input_data, is_gpu)
                    start_time = time.time()

                    s.update("Retrieving pod details...")
                    pod_info = await client.get_pod_details(pod_id)
                    ...
                    if pod_info:
                        ...
                        if is_gpu:
                            ...
                            hw_name = (
                                gpu_info["displayName"] if gpu_info else "NVIDIA GPU"
                            )
                        else:
                    ...
                    ssh_cmd = f"ssh -p {LOCAL_SSH_PORT} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=QUIET 127.0.0.1"
                    child = pexpect.spawn(
                        ssh_cmd, encoding="utf-8", dimensions=(rows, cols)
                    )
